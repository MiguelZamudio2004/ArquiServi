    ...
    <div class="acciones-usuario">
        <div class="notificaciones-container">
            ...
            <div class="notificaciones-dropdown" id="notificacionesDropdown">
                <div class="notificaciones-header"><h3>Notificaciones</h3></div>
                    ...
                    @forelse(auth()->user()->notifications()->latest()->take(10)->get() as $notificacion)
                        <button type="button" class="notificacion {{ $notificacion->read_at ? 'leida' : 'no-leida' }}" data-url="{{ route('notificaciones.leer', $notificacion->id) }}">
                            <div class="notificacion-contenido">
                                <span class="notificacion-titulo">{{ $notificacion->data['titulo'] ?? 'Notificación' }}</span>
                                <p class="notificacion-mensaje">{{ $notificacion->data['mensaje'] ?? '' }}</p>
                                <span class="notificacion-fecha">{{ $notificacion->created_at->diffForHumans() }}</span>
                            </div>
...
<main class="catalogo">
    <section class="catalogo-encabezado">
        ...
        <p>Explora profesionales, proveedores y usuarios registrados en ArquiServi.</p>
    </section>

    <section class="buscador">
        <form action="{{ route('usuarios.buscar') }}" method="GET">
            <div class="campo-busqueda">
                <input type="text" name="buscar" value="{{ $busqueda }}" placeholder="Buscar por nombre, profesión, especialidad, material o ubicación...">
            ...
            <div class="filtros">
                <select name="tipo">
                    ...
                    <option value="usuario" {{ $tipo === 'usuario' ? 'selected' : '' }}>Usuarios</option>
                    <option value="profesional" {{ $tipo === 'profesional' ? 'selected' : '' }}>Profesionales</option>
                ...
                <select name="profesion_id" id="profesion_id">
                    ...
                    @foreach($profesiones as $profesion)
                        <option value="{{ $profesion->id }}" {{ $profesionId == $profesion->id ? 'selected' : '' }}>{{ $profesion->nombre }}</option>
                    @endforeach
                </select>
                ...
                <select name="especialidad_id" id="especialidad_id">
                    ...
                    @foreach($profesiones as $profesion)
                        @foreach($profesion->especialidades as $especialidad)
                            <option value="{{ $especialidad->id }}" data-profesion="{{ $profesion->id }}" {{ $especialidadId == $especialidad->id ? 'selected' : '' }}>{{ $especialidad->nombre }}</option>
                        @endforeach
                    @endforeach
                </select>

                @if($busqueda || $tipo || $profesionId || $especialidadId)
                    <a href="{{ route('usuarios.buscar') }}" class="btn-limpiar">Limpiar filtros</a>
                @endif
            </div>
        </form>
    </section>
    ...
    <section class="resultados">
        <div class="resultados-header">
            <h2>Perfiles</h2>
            <span>{{ $usuarios->total() }} {{ $usuarios->total() === 1 ? 'resultado' : 'resultados' }}</span>
        </div>
        ...
        @if($usuarios->count())
                    ...
                    <article class="usuario-card">
                        <div class="usuario-foto">
                            @if($usuario->foto_perfil)
                                <img src="{{ asset('storage/' . $usuario->foto_perfil) }}" alt="Foto de {{ $usuario->nombre }}">
                            @else
                        ...
                        <div class="usuario-info">
                            <span class="usuario-rol">{{ ucfirst($usuario->rol->nombre) }}</span>
                            ...
                            @if($usuario->rol->nombre === 'profesional' && $usuario->profesional)
                                @if($usuario->profesional->profesiones->isNotEmpty())
                                    <p class="usuario-profesion">{{ $usuario->profesional->profesiones->pluck('nombre')->join(', ') }}</p>
                                @endif

                                @if($usuario->profesional->especialidades->isNotEmpty())
                                    <p class="usuario-especialidad">{{ $usuario->profesional->especialidades->pluck('nombre')->join(', ') }}</p>
                                @endif

                                @if($usuario->profesional->servicios->isNotEmpty())
                                    <p class="usuario-profesion">
                                        <strong>Servicios:</strong> {{ $usuario->profesional->servicios->pluck('nombre')->take(2)->join(', ') }}
                            ...
                            @elseif($usuario->rol->nombre === 'proveedor' && $usuario->proveedor)
                                @if($usuario->proveedor->materiales->isNotEmpty())
                                    <p class="usuario-profesion">{{ $usuario->proveedor->materiales->pluck('nombre')->take(3)->join(', ') }}</p>
                                @endif
                            @endif

                            @if($usuario->ubicacion)
                                <p class="usuario-ubicacion">📍 {{ $usuario->ubicacion }}</p>
                            @endif

                            @php
                                $descripcion = $usuario->descripcion;
                                if (!$descripcion && $usuario->rol->nombre === 'profesional') $descripcion = $usuario->profesional?->descripcion;
                                if (!$descripcion && $usuario->rol->nombre === 'proveedor') $descripcion = $usuario->proveedor?->descripcion;
                            @endphp

                            @if($descripcion)
                                <p class="usuario-descripcion">{{ \Illuminate\Support\Str::limit($descripcion, 120) }}</p>
                            @else
                                <p class="usuario-descripcion sin-descripcion">Sin descripción.</p>
                            @endif
                        </div>

                        <a href="{{ route('perfil.publico', $usuario) }}" class="btn-ver-perfil">Ver perfil</a>
                    </article>
                @endforeach
            </div>
            ...
            @if($usuarios->hasPages())
                <div class="paginacion">
                    ...
                    <span class="pagina-actual">Página {{ $usuarios->currentPage() }} de {{ $usuarios->lastPage() }}</span>
